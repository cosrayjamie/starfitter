...
main (void)
{
  ...
  for (site = 0; site < NSITES; site++)
      ...
      for (mir = MINMIR[site]; mir <= MAXMIR[site]; mir++)
	{
	  ...
	  zhat.e =
	    cos (elev[site][mir] * DEG2RAD) * cos (azi[site][mir] * DEG2RAD);
	  zhat.n =
	    cos (elev[site][mir] * DEG2RAD) * sin (azi[site][mir] * DEG2RAD);
	  ...
	  xhat.n =
	    -(cos (rot[site][mir] * DEG2RAD) *
	      cos (azi[site][mir] * DEG2RAD) +
	      sin (rot[site][mir] * DEG2RAD) * sin (elev[site][mir] *
						    DEG2RAD) *
	      sin (azi[site][mir] * DEG2RAD));
	  xhat.u =
	    sin (rot[site][mir] * DEG2RAD) * cos (elev[site][mir] * DEG2RAD);

	  yhat.e =
	    -(sin (rot[site][mir] * DEG2RAD) *
	      sin (azi[site][mir] * DEG2RAD) +
	      cos (rot[site][mir] * DEG2RAD) * sin (elev[site][mir] *
						    DEG2RAD) *
	      cos (azi[site][mir] * DEG2RAD));
	  yhat.n =
	    ...
	    sin (azi[site][mir] * DEG2RAD);
	  yhat.u =
	    cos (rot[site][mir] * DEG2RAD) * cos (elev[site][mir] * DEG2RAD);
	  ...
	  for (st = 0; st < 256; st++)
	    {
	      ...
	      xc =
		((double) ((row + 1) % 2) * 0.5e0 + (double) col) * DX[site] -
		x0;
	      ...
	      dist =
		sqrt (that.e * that.e + that.n * that.n + that.u * that.u);
		...
		phi += 360.0e0;
	      ...
	      printf
		("%d %d %d %d %d %d %d %d %.12lf %.12lf %.12lf %.3lf %.3lf\n",
