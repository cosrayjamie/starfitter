...
int mirproj (int site, int mir, double star_elev, double star_azi,
	     double *star_x, double *star_y);

const int MAXMIR[NSITES] = { 14, 11, 11 };
...
const double longitude[NSITES] =
  { -112.99429273849e0, -112.71180369861e0, -113.12143272060e0 };
const double latitude[NSITES] =
  { 39.47294139643e0, 39.18834810787e0, 39.20792991505e0 };
...
const double CLUX[NSITES] = { 15.0e0, 22.0e0, 22.0e0 };
const double CLUY[NSITES] = { 15.0e0, 22.0e0, 22.0e0 };
...
main (int argc, char *argv[])
{
  ...
  while (fgets (buffer, 1023, op) != NULL)
      ...
      if (sscanf (buffer, "%d %d", &site, &mir) != 2)
	{
      ...
      if (sscanf
	  (buffer, "%*d %*d %lf %lf %lf %lf %lf", &azi[site][mir],
	   &elev[site][mir], &rot[site][mir], &mcd[site][mir],
	   &Rmir[site][mir]) != 5)
	{
	  fprintf (stderr, "Error reading mirror_geometry.dat\7\7\n");
	  exit (-1);
  ...
  for (site = 0; site < NSITES; site++)
      ...
      for (mir = 0; mir < NMIRS; mir++)
	{
	  ...
	  if (azi[site][mir] < 0.0e0)
	    azi[site][mir] += 360.0e0;
	}
    }
  ...
  if (argc < 10)
    {
      fprintf (stderr,
	       "%s site mir yyyy mm dd hh mm ss.sss tt.ttt pp.ppp\7\7\7\\n\n",
	       argv[0]);
      fprintf (stderr, "site   = Site number, MD=0, BR=1, LR=2\n");
  ...
  seconds = atof (argv[8]);
  if ((seconds < 0.0e0) || (seconds >= 60.0e0))
    exit (-1);

  temperature = atof (argv[9]);
  if ((temperature < -50.0e0) || (temperature > 50.0e0))
    exit (-1);

  pressure = atof (argv[10]);
  if ((pressure < 800.0e0) || (pressure > 870.0e0))
    exit (-1);
  ...
  make_on_surface (clf_latitude, clf_longitude, clf_height, temperature,
		   pressure, &geo_loc);
  make_observer_on_surface (clf_latitude, clf_longitude, clf_height,
			    temperature, pressure, &obs_loc);

  if ((fp = fopen ("bsc5.cat", "r")) == NULL)
    exit (-1);
  ...
  while ((fgets (buffer, 2047, fp) != NULL) && (i < NSTAR))
    {
      if (sscanf (buffer, " %d %d %d %lf %d %d %lf %lf %lf %lf%lf %lf %s",
		  ...
		  &ra_pm, &dec_pm, &rv, &parallax, &vmag[i], sname) != 13)
	continue;
      ...
      else
	{
      ...
      if (make_cat_entry (sname, "BSC5", (long int) i,
			  (double) ra_hh + ((double) ra_mm +
					    ra_ss / 60.0e0) / 60.0e0, decval,
			  ra_pm * 10.0e0, dec_pm * 10.0e0, parallax * 1.0e4,
			  rv, &star[i]) != 0)
	exit (-1);
      i++;
    }
...
*/
  ...
  for (i = 0; i < NSTAR; i++)
	...
	continue;
      if ((error =
	   topo_star (jd_tt, delta_t, &star[i], &geo_loc, accuracy, &rat,
		      &dect)) != 0)
	{
      ...
      if (fabs (elev[site][mir] - (90.0e0 - zd)) < 20.0e0)
	{
	  if (fabs (azi[site][mir] - az) < 30.0e0)
		...
		{
		  if ((fabs (star_x) < CLUX[site])
		      && (fabs (star_y) < CLUY[site]))
		    {
		      printf ("%s %.3lf %.3lf %.5lf %.5lf %.2lf\n",
			      star[i].starname, 90.0e0 - zd, az, star_x,
			      star_y, vmag[i]);
		    }
		}
	    }
	  ...
	  if (fabs (azi[site][mir] - az) < 30.0e0)
	    {
	      if (mirproj (site, mir, 90.0e0 - zd, az, &star_x, &star_y) == 0)
		{
		  if ((fabs (star_x) < 1.01e0 * CLUX[site])
		      && (fabs (star_y) < CLUY[site]))
		    {
		      printf ("%s %.3lf %.3lf %.2lf %.2lf %.2lf\n",
			      planet[i].name, 90.0e0 - zd, az, star_x, star_y,
			      pmag[i]);
		    }
		}
	    }
...
int
mirproj (int site, int mir, double star_elev, double star_azi, double *star_x,
	 double *star_y)
{
  double z0;
  ...
  xhat.e =
    cos (rot[site][mir] * DEG2RAD) * sin (azi[site][mir] * DEG2RAD) -
    sin (rot[site][mir] * DEG2RAD) * sin (elev[site][mir] * DEG2RAD) *
    cos (azi[site][mir] * DEG2RAD);
  xhat.n =
    -(cos (rot[site][mir] * DEG2RAD) * cos (azi[site][mir] * DEG2RAD) +
      sin (rot[site][mir] * DEG2RAD) * sin (elev[site][mir] * DEG2RAD) *
      sin (azi[site][mir] * DEG2RAD));
  xhat.u = sin (rot[site][mir] * DEG2RAD) * cos (elev[site][mir] * DEG2RAD);

  yhat.e =
    -(sin (rot[site][mir] * DEG2RAD) * sin (azi[site][mir] * DEG2RAD) +
      cos (rot[site][mir] * DEG2RAD) * sin (elev[site][mir] * DEG2RAD) *
      cos (azi[site][mir] * DEG2RAD));
  yhat.n =
    sin (rot[site][mir] * DEG2RAD) * cos (azi[site][mir] * DEG2RAD) -
    cos (rot[site][mir] * DEG2RAD) * sin (elev[site][mir] * DEG2RAD) *
    sin (azi[site][mir] * DEG2RAD);
