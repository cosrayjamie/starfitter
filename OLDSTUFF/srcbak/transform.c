...
main (int argc, char *argv[])
{
  ...
  while (fgets (buffer, 1024, lp) != NULL)
      ...
      while (fgets (buffer, 1023, fp) != NULL)
	{
	  ...
	  mtype = 0;
	  if (strcasestr (buffer, "center") != NULL)
	    mtype = 1;
	  else if (strcasestr (buffer, "corner") != NULL)
	    mtype = 2;
	  else if (strcasestr (buffer, "star") != NULL)
	    mtype = 3;
	  ...
	  while ((token = strtok (NULL, ", ;\t")) != NULL)
	    {
	      ...
	      if (MX == 0)
		{
		  if (strstr (token, "XM") != NULL)
		    MX = M;
		}
	      if (MY == 0)
		{
		  if (strstr (token, "YM") != NULL)
		    MY = M;
		}
      ...
      if (day < 1)
	{
      ...
      sprintf (fname,
	       "./ta_stars %d %d %d %d %d %d %d %.2lf %.2lf %.2lf | sort -k 6 -g",
