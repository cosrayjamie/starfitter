...
double xglobal[10];   // Global storage of x parameter vector
...
int main(int argc, char *argv[])
{
  ...
  double stheta, sphi;
  double xstar, ystar;
  double theta, phi, psi, x0, y0, D0, D1;
  double chi2min;
  char buffer[1025];
  ...
  for(iboot=0;iboot<NBOOT;iboot++)
	...
	parms[3] = 0.0e0;
      ...
      if(NPARMS>4)
	parms[4] = D1 = (drand48()-0.5e0)/100.0e0;
      else
	parms[4] = D1 = 0.0e0;
      
      if(NPARMS>5)
	parms[5] = x0 = drand48()-0.5e0;
      else
	parms[5] = x0 = 0.0e0;
      
      if(NPARMS>6)
	parms[6] = y0 = drand48()-0.5e0;
      else
	parms[6] = y0 = 0.0e0; 
      
      for(i=0;i<7;i++)xglobal[i+1] = (float)parms[i]; // Save all parameters globally
      ...
      for(iter=0;iter<4;iter++)
	{
      ...
      printf("%.4lf ", mcd[site][mir]+D0*2.54e0/100.0e0);  // D0
      if(NPARMS>4)printf("%.4lf ", D1*2.54e0/100.0e0);  // D1
      if(NPARMS>5)printf("%.4lf ", x0);
      if(NPARMS>6)printf("%.4lf ", y0);
      printf("%d ", Nstars);
      printf("%.8lf\n", chi2min);
      fflush(stdout);
    }
...
{
  VECTOR m, xp, yp;
  double theta, phi, psi, x0, y0, D0, D1, Dprime;
  double tanO;
  ...
  tanO  = tan(acos(dot(s[istar],m)));
	      
  Dprime = (D0 + D1*tanO); // Nonlinear model for plane distance

  *xstar = Dprime*dot(s[istar],xp)/dot(s[istar],m)+x0;
  *ystar = Dprime*dot(s[istar],yp)/dot(s[istar],m)+y0;
