...
main (int argc, char *argv[]) 
{
    ...
    
    {
	   ...
	   (buffer,
	    " %d %d %d/%d/%d %d:%d:%lf %d %s %lf %lf %lf %lf %lf %lf %lf",
	...
	if (fabs (xstar) > CUTX[site])
	continue;
      if (fabs (ystar) > CUTY[site])
	continue;
      if (fabs (xmeas[Nstars]) > CUTX[site])
	continue;
      if (fabs (ymeas[Nstars]) > CUTY[site])
	continue;
...
fit_func (int N, double param[], float (*funk) (float[])) 
{
...
starpos (int istar, double *xstar, double *ystar, float x[]) 
{
