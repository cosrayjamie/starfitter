...
/* allocate a float vector with subscript range v[nl..nh] */
{
  ...
  v =
    (float *)
    malloc ((unsigned int) ((nh - nl + 1 + NR_END) * sizeof (float)));
  if (!v)
    nrerror ("allocation failure in vector()");
  return v - nl + NR_END;
}
...
/* allocate a double matrix with subscript range m[nrl..nrh][ncl..nch] */
{
  ...
  /* allocate pointers to rows */
  m =
    (double **) malloc ((unsigned int) ((nrow + NR_END) * sizeof (double *)));
  if (!m)
    nrerror ("allocation failure 1 in matrix()");
  m += NR_END;
  m -= nrl;
...
float ***
f3tensor (nrl, nrh, ncl, nch, ndl, ndh)
...
/* allocate a float 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
  ...
  /* allocate pointers to pointers to rows */
  t =
    (float ***) malloc ((unsigned int) ((nrow + NR_END) * sizeof (float **)));
  if (!t)
    nrerror ("allocation failure 1 in f3tensor()");
  t += NR_END;
  t -= nrl;
